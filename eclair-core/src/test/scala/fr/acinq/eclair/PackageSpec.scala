...
class PackageSpec extends FunSuite {
  ...
  test("decode bech32 addresses") {
    ...
    // p2wpkh
    assert(addressToPublicKeyScript(Bech32.encodeWitnessAddress("via", 0, pub.hash160), Block.LivenetGenesisBlock.hash) == Script.pay2wpkh(pub))
    assert(addressToPublicKeyScript(Bech32.encodeWitnessAddress("tvia", 0, pub.hash160), Block.TestnetGenesisBlock.hash) == Script.pay2wpkh(pub))
    assert(addressToPublicKeyScript(Bech32.encodeWitnessAddress("viart", 0, pub.hash160), Block.RegtestGenesisBlock.hash) == Script.pay2wpkh(pub))

    // wrong version
    assert(Try(addressToPublicKeyScript(Bech32.encodeWitnessAddress("via", 1, pub.hash160), Block.LivenetGenesisBlock.hash)).isFailure)
    assert(Try(addressToPublicKeyScript(Bech32.encodeWitnessAddress("via", 1, pub.hash160), Block.TestnetGenesisBlock.hash)).isFailure)
    assert(Try(addressToPublicKeyScript(Bech32.encodeWitnessAddress("viart", 1, pub.hash160), Block.RegtestGenesisBlock.hash)).isFailure)

    // wrong chain
    assert(Try(addressToPublicKeyScript(Bech32.encodeWitnessAddress("via", 0, pub.hash160), Block.TestnetGenesisBlock.hash)).isFailure)
    assert(Try(addressToPublicKeyScript(Bech32.encodeWitnessAddress("tvia", 0, pub.hash160), Block.LivenetGenesisBlock.hash)).isFailure)
    assert(Try(addressToPublicKeyScript(Bech32.encodeWitnessAddress("viart", 0, pub.hash160), Block.LivenetGenesisBlock.hash)).isFailure)

    val script = Script.write(Script.pay2wpkh(pub))
    assert(addressToPublicKeyScript(Bech32.encodeWitnessAddress("via", 0, Crypto.sha256(script)), Block.LivenetGenesisBlock.hash) == Script.pay2wsh(script))
    assert(addressToPublicKeyScript(Bech32.encodeWitnessAddress("tvia", 0, Crypto.sha256(script)), Block.TestnetGenesisBlock.hash) == Script.pay2wsh(script))
    assert(addressToPublicKeyScript(Bech32.encodeWitnessAddress("viart", 0, Crypto.sha256(script)), Block.RegtestGenesisBlock.hash) == Script.pay2wsh(script))
  }
