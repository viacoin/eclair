...
trait ElectrumxService extends DockerTestKit {
  ...
  val electrumxContainer = if (System.getProperty("os.name").startsWith("Linux")) {
    ...
    DockerContainer("acinq/electrumx")
      ...
      .withEnv("DAEMON_URL=http://foo:bar@localhost:28332", "COIN=Viacoin", "NET=regtest", s"TCP_PORT=$electrumPort")
    ...
    DockerContainer("acinq/electrumx")
      ...
      .withEnv("DAEMON_URL=http://foo:bar@host.docker.internal:28332", "COIN=Viacoin", "NET=regtest", s"TCP_PORT=$electrumPort")
      //.withLogLineReceiver(LogLineReceiver(true, println))
  }
