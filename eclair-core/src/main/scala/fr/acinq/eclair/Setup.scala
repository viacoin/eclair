...
class Setup(datadir: File,
  ...
  val bitcoin = nodeParams.watcherType match {
    case BITCOIND =>
      ...
      assert(progress > 0.8, "viacoind should be synchronized (progress=$progress")
      assert(headers - blocks <= 1, "viacoind should be synchronized (headers=$headers blocks=$blocks")
